...
class DrachtioAgent extends Emitter {
  ...
  /**
   * High-availability re-attach: after a failover the application reconnects to the new
   * active drachtio (e.g. via a floating VIP) and tells it which dialogs it still owns,
   * so that in-dialog requests (re-INVITE / BYE) route back to this app on the new server.
   * @param {Socket} socket - the (reconnected) socket; defaults to the current default socket
   * @param {string[]} dialogIds - stack dialog ids to re-attach
   * @param {function} callback - (err, {response})
   */
  reattach(socket, dialogIds, callback) {
    if (!typeSocket(socket)) {
      callback = dialogIds;
      dialogIds = socket;
      socket = this._getDefaultSocket();
    ...
    const obj = this.mapServer.get(socket);
    if (!obj) {
      if (callback) callback(new Error('reattach: socket not found'));
      return;
    }
    if (!Array.isArray(dialogIds)) dialogIds = [dialogIds];
    if (dialogIds.length === 0) {
      if (callback) callback(null, {response: 'reattached 0 of 0'});
      return;
    }
    const msgId = this.wp.send(socket, `reattach|${dialogIds.join(',')}`);
    obj.pendingRequests.set(msgId, (token) => {
      if ('OK' === token[0]) {
        if (callback) callback(null, {response: token[1]});
      }
      else if (callback) callback(new Error(token[1] || 'reattach failed'));
    });
    return msgId;
  }
    ...
    obj.pendingRequests.set(msgId, (response) => {
      ...
      if (obj.authenticated) {
        ...
        if (this.wp.isClient) {
          ...
          // detect a reconnect (failover): a prior connection had already succeeded
          const reconnected = this._everConnected === true;
          this._everConnected = true;
          setImmediate(() => {
            this.emit('connect', null, obj.hostport, obj.serverVersion, obj.localHostports);
            // signal HA layer to re-attach dialogs on the new socket
            if (reconnected) this.emit('reconnected', socket);
          });
        }
        else {
          this.emit('connect', null, obj.hostport, obj.serverVersion, obj.localHostports);
          ...
          if ('network' === source && sipMsg.type === 'request') {
            ...
            if ('INVITE' === req.method) {
              ...
              if (inviteReq.listenerCount('update') > 0) {
                ...
                debug(`Agent#handle - emitted update event for INVITE with call-id ${callId}`);
                return;
              }
              else {
                res.send(500, {
                ...
                debug(`Agent#handle - no listeners for update event for INVITE with call-id ${callId}`);
                return;
              }
            }
            else if ('UPDATE' === req.method) {
              // handle UPDATE for UAC INVITE
              // UPDATE and INVITE share same call-id, but not the same transaction id
              // so we need to find the pending sip request with the same call-id
              obj.pendingSipRequests.forEach((value, _key) => {
                if (value.req.get('call-id') === callId) {
                  debug(`Agent#handle - found pending sip request with call-id ${callId} and key ${_key}`);
                  sr = value;
                }
              });
              // if we found a pending sip request with the same call-id, emit update event
              if (sr) {
                debug(`Agent#handle - got UPDATE for UAC INVITE with call-id ${callId}`);
