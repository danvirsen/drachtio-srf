...
class Dialog extends Emitter {
  ...
  get id() {
    return this._id !== undefined ? this._id : this.res.stackDialogId;
  }

  get dialogType() {
    return this._dialogType !== undefined ? this._dialogType : this.req.method;
  }

  get subscribeEvent() {
    if (this._subscribeEvent !== undefined) return this._subscribeEvent;
    return this.dialogType === 'SUBSCRIBE' ? this.req.get('Event') : null;
  }

  get socket() {
    return this._socket || (this.req && this.req.socket);
  }

  /**
   * Opaque application business state attached to this dialog. Persisted with the dialog
   * record (approach B) so it can be restored after an application restart. Setting it
   * re-persists the dialog if a dialog store is enabled on the Srf instance.
   */
  get metadata() {
  ...
  /**
   * Serialize this dialog to a plain record suitable for persisting to an external store
   * (e.g. redis) and later rebuilding via Dialog.fromState() after an application restart.
   */
  toState() {
    return {
      id: this.id,
  ...
  /**
   * Re-persist this dialog to the Srf dialog store, if one is enabled. Best-effort; call
   * after mutating dialog state (e.g. SDP via modify(), or metadata) so a recovery after an
   * app restart sees current values.
   */
  persist() {
    if (this.srf && typeof this.srf._persistDialog === 'function') this.srf._persistDialog(this);
    return this;
  }

  /**
   * Rebuild a Dialog from a record produced by toState(). Used by Srf#recoverDialogs() after
   * an application restart: the SIP plumbing is reconstructed from the record and routing is
   * re-bound to the drachtio server via the reattach control verb. The application's business
   * state rides along in state.metadata.
   * @param {Srf} srf
   * @param {Object} state - a record previously produced by Dialog#toState()
   * @returns {Dialog}
   */
  static fromState(srf, state) {
    const dlg = Object.create(Dialog.prototype);
    Emitter.call(dlg);
    ...
    dlg.type = state.type;
    dlg._id = state.id;
    dlg._dialogType = state.dialogType;
    dlg._subscribeEvent = state.subscribeEvent || null;
    dlg.auth = state.auth;
    dlg.onHold = !!state.onHold;
    dlg.connected = true;
    dlg.queuedRequests = [];
    dlg._queueRequests = false;
    dlg._reinvitesInProgress = { count: 0, admitOne: [] };
    dlg.sip = state.sip || {};
    dlg.local = state.local || {};
    dlg.remote = state.remote || {};
    dlg.subscriptions = state.subscriptions || [];
    dlg._metadata = state.metadata;
    dlg._recovered = true;
    return dlg;
  }

  /**
   * Rebind this dialog to a new drachtio connection socket. Used by Srf after a
   * high-availability failover: the application reconnects to the (new active) server
   * and re-attaches its dialogs, so in-dialog requests we originate (re-INVITE, BYE)
   * must go out on the new socket rather than the destroyed one.
   */
  setSocket(socket) {
    this._socket = socket;
  }
