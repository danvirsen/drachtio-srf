...
class Srf extends Emitter {
  ...
  constructor(app) {
    ...
    if (!this._app) {
      ...
      // high availability: when the agent reconnects after a failover, re-attach our
      // live dialogs to the new server so in-dialog requests keep routing to this app
      this._app.on('reconnected', (socket) => this._reattachDialogs(socket));
    ...
    const __send = (content, callback) => {
      ...
      return res.send(req.method === 'INVITE'  ? 200 : 202, {
        ...
        try {
          ...
          debug(`createUAS: error creating dialog: ${err}`);
          if (!called) {
            called = true;
            callback(err);
          }
                  ...
                  return callback(null, {
                    ...
                    ack: (localSdp) => {
                      return new Promise((resolve, reject) => {
                        ...
                        let dialog;
                        try {
                          dialog = new Dialog(this, 'uac', {req: req, res: res, auth: opts.auth});
                        } catch(err) {
                          debug(`createUAC: error creating dialog: ${err}`);
                          return reject(err);
                        }
        ...
        if (propagateProvisional) {
          ...
          if (provisionalRes.body) {
            try {
              ...
              opts.body = sdpA;
              return res.send(provisionalRes.status, provisionalRes.reason, opts, null, fnAck);
            } catch(err) {
            ...
            res.send(provisionalRes.status, provisionalRes.reason, opts, null, fnAck);
          }
        }
        else {
    ...
    const __x = async(callback) => {
      ...
      try {
        ...
        if (propagateFailure && !res.finalResponseSent) {
          ...
          res.send(err.status || 500, err.reason, opts);
        }
        return callback(err);
      }
  ...
  * @returns {Srf|Promise} returns a Promise if no callback is supplied, otherwise the Srf object
  ...
  addDialog(dialog) {
    ...
    this._persistDialog(dialog);
  }

  removeDialog(dialog) {
    this._dialogs.delete(dialog.id);
    debug('Srf#removeDialog: removing dialog with id %s dialog count is now %d', dialog.id, this._dialogs.size);
    this._unpersistDialog(dialog.id);
  }

  /**
   * Enable application-restart resilience (approach B). The application supplies a store that
   * persists each dialog record (SIP plumbing + opaque business `metadata`) keyed by dialog id.
   * On restart, call {@link Srf#recoverDialogs} (or pass `recoverOnConnect`) to rebuild the
   * Dialog objects from the store and re-attach them to drachtio so in-dialog requests keep
   * routing to this app.
   *
   * The store must implement (sync or Promise-returning):
   *   - set(dialogId, record)   persist/overwrite a dialog record
   *   - del(dialogId)           remove a dialog record
   *   - list()                  return an array of all this instance's dialog records
   *
   * The store is responsible for namespacing records to THIS application instance (e.g. by an
   * instance id) so list() only returns dialogs this instance owns. See docs/dialog-persistence.md.
   *
   * @param {Object} store - persistence backend (e.g. redis-backed)
   * @param {Object} [opts]
   * @param {boolean} [opts.recoverOnConnect=false] - automatically recover on the next connect
   * @returns {Srf}
   */
  enableDialogPersistence(store, opts) {
    assert.equal(typeof store, 'object', 'Srf#enableDialogPersistence: store is required');
    ['set', 'del', 'list'].forEach((m) =>
      assert.equal(typeof store[m], 'function', `Srf#enableDialogPersistence: store must implement ${m}()`));
    this._dialogStore = store;
    this._dialogStoreOpts = opts || {};
    if (this._dialogStoreOpts.recoverOnConnect) {
      // recover once, on the first connect after persistence is enabled
      const onConnect = () => { this.recoverDialogs().catch((err) => this.emit('recover-failed', err)); };
      this._app.on('connect', onConnect);
    }
    return this;
  }

  _persistDialog(dialog) {
    if (!this._dialogStore || dialog._recovered === 'pending') return;
    Promise.resolve()
      .then(() => this._dialogStore.set(dialog.id, dialog.toState()))
      .catch((err) => debug(`Srf#_persistDialog: error persisting ${dialog.id}: ${err}`));
  }

  _unpersistDialog(dialogId) {
    if (!this._dialogStore) return;
    Promise.resolve()
      .then(() => this._dialogStore.del(dialogId))
      .catch((err) => debug(`Srf#_unpersistDialog: error removing ${dialogId}: ${err}`));
  }

  /**
   * Recover dialogs after an application restart: read persisted dialog records, rebuild Dialog
   * objects (SIP plumbing + business `metadata`), and re-attach them to drachtio so in-dialog
   * requests route back to this app. Requires {@link Srf#enableDialogPersistence} first.
   *
   * Emits 'dialogRecovered' for each rebuilt dialog so the application can re-wire its business
   * logic (event listeners, timers, bridge references) from `dialog.metadata`.
   *
   * @param {Socket} [socket] - the control socket to bind to (defaults to the current one)
   * @returns {Promise<Dialog[]>} the recovered dialogs
   */
  async recoverDialogs(socket) {
    assert.ok(this._dialogStore, 'Srf#recoverDialogs: call enableDialogPersistence() first');
    socket = socket || (this._app._getDefaultSocket && this._app._getDefaultSocket());

    const records = await this._dialogStore.list();
    const recovered = [];
    for (const rec of (records || [])) {
      try {
        const dlg = Dialog.fromState(this, rec);
        if (socket) dlg.setSocket(socket);
        // mark so _persistDialog from addDialog does not rewrite an identical record mid-recovery
        dlg._recovered = 'pending';
        this._dialogs.set(dlg.id, dlg);
        recovered.push(dlg);
        this.emit('dialogRecovered', dlg);
        dlg._recovered = true;
      } catch(err) {
        debug(`Srf#recoverDialogs: failed to rebuild a dialog: ${err}`);
        this.emit('recover-failed', err);
      }
    }
    debug(`Srf#recoverDialogs: rebuilt ${recovered.length} dialog(s); re-attaching`);
    if (recovered.length && socket) this._reattachDialogs(socket);
    return recovered;
  }

  /**
   * High-availability re-attach. Invoked when the underlying connection reconnects after a
   * failover. Points every live dialog at the new socket and tells the (recovered) server
   * which dialog ids this app still owns, so in-dialog requests route back here.
   *
   * The newly-promoted server may still be loading dialogs from redis when we reconnect, so
   * if not all dialogs are accepted we retry a few times with a short backoff.
   *
   * Emits 'reattached' on success and 'reattach-failed' on error.
   * @private
   */
  _reattachDialogs(socket, attempt = 0) {
    if (this._dialogs.size === 0) {
      debug('Srf#_reattachDialogs: no live dialogs to re-attach');
      return;
    }
    const dialogIds = [...this._dialogs.keys()];

    // repoint existing dialogs at the new connection so re-INVITE/BYE we originate go out on it
    for (const dialog of this._dialogs.values()) {
      if (typeof dialog.setSocket === 'function') dialog.setSocket(socket);
    }

    debug(`Srf#_reattachDialogs: re-attaching ${dialogIds.length} dialog(s), attempt ${attempt + 1}`);
    this._app.reattach(socket, dialogIds, (err, result) => {
      if (err) {
        if (attempt < 5) {
          setTimeout(() => this._reattachDialogs(socket, attempt + 1), 500);
          return;
        }
        debug(`Srf#_reattachDialogs: failed after ${attempt + 1} attempts: ${err}`);
        this.emit('reattach-failed', err);
        return;
      }
      // server replies "reattached N of M"; retry if the server has not yet recovered them all
      const m = /reattached\s+(\d+)\s+of\s+(\d+)/.exec(result && result.response || '');
      if (m && parseInt(m[1], 10) < parseInt(m[2], 10) && attempt < 5) {
        debug(`Srf#_reattachDialogs: partial (${result.response}); retrying`);
        setTimeout(() => this._reattachDialogs(socket, attempt + 1), 500);
        return;
      }
      debug(`Srf#_reattachDialogs: ${result && result.response}`);
      this.emit('reattached', {count: dialogIds.length, response: result && result.response});
    });
  }
